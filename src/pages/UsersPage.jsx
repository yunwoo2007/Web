    ...
    return (
        <div className="flex flex-col items-center p-6 bg-gray-100 min-h-screen">
            <div className="flex justify-between w-full max-w-6xl mb-6">
                ...
                <button className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg shadow-md transition" onClick={() => setIsAddUserModalOpen(true)}>
                    + Add User
                </button>
            </div>
                ...
                <tbody>
                    {users.map((user, index) => (
                        <tr key={user.id} className={`border-b border-red-600 ${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                            ...
                            <td className="px-4 py-3 border-r border-red-600">{user.subject?.join(", ") || "No subjects assigned"}</td>
                            <td className="px-4 py-3">
                                <button onClick={() => handleDeleteUser(user.id)} className="mr-2 text-red-600 hover:text-red-800">
                                    <FiTrash2 />
                                </button>
                                <button onClick={() => { setExpandedUser(user.id); setIsSubjectModalOpen(true); }}>
